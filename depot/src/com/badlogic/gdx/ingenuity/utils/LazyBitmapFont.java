...
public class LazyBitmapFont extends BitmapFont {
	...
	public LazyBitmapFont(FreeTypeFontGenerator generator, int fontSize) {
		...
		this.parameter.size = fontSize;
		this.data = new LazyBitmapFontData();
		try {
	...
	private class LazyBitmapFontData extends FreeTypeBitmapFontData {
		int page = 1;
		...
		private Glyph generateGlyph(char ch) {
			GlyphAndBitmap gab = generator.generateGlyphAndBitmap(ch, parameter.size, false);
			if (gab == null || gab.bitmap == null)
				return null;

			Pixmap map = gab.bitmap.getPixmap(Format.RGBA8888, parameter.color, parameter.gamma);
